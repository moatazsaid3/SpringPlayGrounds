...
public interface CourseRepository extends JpaRepository<Course, UUID> {
    ...
    @Query ("""
            SELECT 
            new com.moataz.springplaygrounds.springdata.dto.CourseInfoDTO (c.name,LISTAGG(s.firstName || ' ' || s.lastName, ', ') WITHIN GROUP (ORDER BY (s.firstName)), c.startDate) 
            FROM Course c 
            JOIN c.students s
            WHERE c.courseLevel = 'beginner'
            GROUP BY c.name, c.startDate 
            """)
    public List<CourseInfoDTO> getCourseNameAndStudentsAdvanced ();
}
